...
def cached_send_file(url, file_data):
    ...
    return send_file(
        file_stream, download_name=file_name, mimetype="application/octet-stream"
    )
    ...
    try:
        ...
        if sta_cached_entry:
            ...
            if filename in lookup_table:
                ...
                filename == "stellarlatexxetex.fmt"
                or filename == "xetexfontlist.txt"
                ...
                url = "/var/lib/texlive/repo/simple/run/" + filename
            else:
            ...
            if useCache:
                if has_file:
                    file_data = get_cached_file_data(url)
                file_status_cache[sta_cache_key] = FileCacheEntry(
                    url, has_file, file_data
                )
        ...
        if url is None or not has_file:
            ...
            response = make_response(
                cached_send_file(url, file_data)
                if useCache
                else no_cache_send_file(url, file_data)
            )
...
@resapp.route("/xetex/pk/<int:dpi>/<filename>")
@cross_origin()
def xetex_fetch_pk(dpi, filename):
    try:
        filename = san(filename)
        url = None
        has_file = False
        file_data = None
        sta_cache_key = f"xetex+pk+{dpi}+{filename}"
        sta_cached_entry = file_status_cache.get(sta_cache_key) if useCache else None
        if sta_cached_entry:
            url = sta_cached_entry.url
            has_file = sta_cached_entry.exists
            ...
            url = pykpathsea_xetex.find_pk(filename, dpi)
            if url is not None:
                has_file = os.path.isfile(url)
            if useCache:
                if has_file:
        ...
        print(f"Error in xetex_fetch_pk: {e}")
        return "Internal Server Error", 500
    ...
    try:
        ...
        if sta_cached_entry:
            ...
            if filename in lookup_table:
                ...
                filename == "stellarlatexpdftex.fmt"
                or filename == "xetexfontlist.txt"
                or filename == "tlpkg.txt"
                or filename == "vfpkg.txt"
                or filename == "pdftex.map"
...
@resapp.route("/luatex/<int:fileformat>/<filename>")
@cross_origin()
def luatex_fetch_file(fileformat, filename):
    try:
        filename = san(filename)
        url = None
        has_file = False
        file_data = None
        sta_cache_key = f"luatex+{fileformat}+{filename}"
        sta_cached_entry = file_status_cache.get(sta_cache_key) if useCache else None
        if sta_cached_entry:
            url = sta_cached_entry.url
            has_file = sta_cached_entry.exists
            ...
            if filename in lookup_table:
                ...
                filename == "stellarlatexluatex.fmt"
                or filename == "xetexfontlist.txt"
                or filename == "tlpkg.txt"
                or filename == "vfpkg.txt"
                or filename == "pdftex.map"
                ...
                url = pykpathsea_luatex.find_file(filename, fileformat)
            if url is not None:
                has_file = os.path.isfile(url)
            if useCache:
                if has_file:
        ...
        print(f"Error in luatex_fetch_file: {e}")
        return "Internal Server Error", 500


@resapp.route("/luatex/pk/<int:dpi>/<filename>")
@cross_origin()
def luatex_fetch_pk(dpi, filename):
    try:
        filename = san(filename)
        url = None
        has_file = False
        file_data = None
        sta_cache_key = f"luatex+pk+{dpi}+{filename}"
        sta_cached_entry = file_status_cache.get(sta_cache_key) if useCache else None
        if sta_cached_entry:
            url = sta_cached_entry.url
            has_file = sta_cached_entry.exists
            ...
            url = pykpathsea_luatex.find_pk(filename, dpi)
            if url is not None:
                has_file = os.path.isfile(url)
            if useCache:
                if has_file:
        ...
        print(f"Error in luatex_fetch_pk: {e}")
        return "Internal Server Error", 500
